...
public boolean registerSupplierQuery(Suppliers supplier){
    String query = "INSERT INTO suppliers (name, description, address, phone, email, city, created, updated)"
            + "VALUES(?,?,?,?,?,?,?,?)";
...
public boolean updateSupplierQuery(Suppliers supplier){
    String query = "UPDATE suppliers SET name = ?, description = ?, address = ?, phone = ?, email = ?, "
            + "city = ?, updated = ? WHERE id = ?";
    ...
    try{
        ...
        JOptionPane.showMessageDialog(null, "Error al modificar datos del proveedor");
        return false;
    }
    }
...
public boolean deleteSupplierQuery(int id){
    ...
    try{
        ...
        JOptionPane.showConfirmDialog(null, "No puedes eliminar un proveedor que tiene relación con otra tabla");
        return false;
    }
  }
}
